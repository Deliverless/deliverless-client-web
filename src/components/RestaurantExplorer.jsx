...
export default function RestaurantExplorer({ restaurants }) {
  ...
  const handleSelectNode = (event) => {
    ...
    let selectedNode = data.nodes.find(
      (n) => n.id === (event.nodes ? event.nodes[0] : event.node)
    );
    if (!selectedNode) return;
    selectedNode.font = { face: "Monospace", align: "left" };
    selectedNode.savedLabel =
      selectedNode.label && selectedNode.label.length > 0
        ? selectedNode.label
        : selectedNode.savedLabel;
    selectedNode.decorator = (props) => {
      return RestaurantDecorator(props, selectedNode.savedLabel);
    };
    selectedNode.label = "";
    setData({
      edges: [...data.edges],
  ...
  const handleDeselectNode = (event) => {
    ...
    let selectedNode = data.nodes.find(
      (n) =>
        n.id ===
        (event.previousSelection?.nodes
          ? event.previousSelection.nodes[0]
          : event.node)
    );
    if (!selectedNode) return;
